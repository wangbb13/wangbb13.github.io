...
public class SocialGenerator {
    ...
    public void genPWL(String[] args) {
        ...
        if (args.length != 13) {
            System.out.println("usage: java SocialGenerator ilambda idmin idmax olambda odmin odmax snodes tnodes edges filename comms clambda param-c");
            return;
        }
        ...
        int nCom = Integer.parseInt(args[10]);
        double cLambda = Double.parseDouble(args[11]);
        double pC = Double.parseDouble(args[12]);
        double rho = pC - 1.0;
        ...
        int[][] split = Utility.splitCommunity((int)nNodes, (int)tNodes, nCom, cLambda);
	...
	System.out.println("# Communities = " + String.valueOf(anComms));
	    ...
	    // System.out.println("Source Node " + String.valueOf(i) + " ... ");
            cumuRow++;
            ...
            if (rand.nextDouble() < rho)
                extraOutDegree = extraDegree(odMax - mainOutDegree + 10, pC);
            ...
            while (spColJ < anComms) {
                ...
                if (spColJ == spRowI) { // main diagonal
                    ...
                    // if (rand.nextDouble() > 0.6) {
                        // num = eachExtraOut;
                        num = (int)Math.round(extraOutDegree * 1.0 * split[spColJ][1] / 1.0 * (nNodes - split[spRowI][1]));
                    // } else {
                        // num = 0;
                    // }
                }
    ...
    public static void main(String[] args) {
        ...
        SocialGenerator gen = new SocialGenerator();
        gen.genPWL(args);
