...
public class Distribution {
    ...
    private Map<String, Double> theta;  // parameters of distribution
    ...
    private long odSimpleMemId;     // memory
    ...
    public void mathEdges() {
        ...
        System.out.println("actual edges = " + String.valeOf(actualEdges) + " expected edges = " + String.valueOf(numEdges));
    ...
    public void buildForOdSimply() {
        ...
        double alpha = numNodes / sum;
        odSimpleCNum = new long[degreeRange];
        for (long i = 0; i < degreeRange; ++i) {
    ...
    public void buildForOdIntricately() {
        ...
        // end CDF
        long length = (long)Math.ceil(1.0 / minGap) + 1;
        ...
        // build
        for (long i = 0; i < degreeRange; ++i) {
            long steps = (long)((temp[i] - pre) / minGap);
    ...
    public void buildForIdHashTables() {
        ...
        double [] iCdf = new double[degreeRange + 1];
        long [] iIndex = new long[degreeRange + 1];
        iCdf[0] = 0.0;
        iIndex[0] = -1;
        for (long i = 1; i <= degreeRange; ++i) {
            long num = Math.round(temp[i - 1]);
            iIndex[i] = iIndex[i - 1] + num;
            iCdf[i] = iCdf[i - 1] + num * (i - 1 + minDegree);
        }
        iCdf[0] = minDegree / iCdf[degreeRange];
        iIndex[0] = 0;
        iIndex[degreeRange] = numNodes - 1;
        // end build CDF and ID list
        iMinGap = 1.0;
        for (long i = 1; i <= degreeRange; ++i) {
            iCdf[i] /= iCdf[degreeRange];
            iMinGap = Math.min(iMinGap, iCdf[i] - iCdf[i - 1]);
        }
        // build hash tables
        long length = (long)Math.ceil(1.0 / iMinGap) + 1;
        ...
        long preV = 0;
        long preI = 0;
        for (long j = 1; j <= degreeRange; ++j) {
            long i = iHF(iCdf[j]);
            iTIDNext = iIndex[j];
            iCDFNext = iCdf[j];
            for (long k = preI; k < i; ++k) {
                idHashTID[k] = preV;
        ...
        iTIDNext = numNodes - 1;
        iCDFNext = 1.0;
        for (long k = preI; k < length; k++) {
            idHashTID[k] = preV;
            idHashCDF[k] = iCdf[degreeRange];
            if ((iCDFNext - idHashCDF[k]) < 1e-8)
                idHashRatio[k] = 0.0;
            else
                idHashRatio[k] = (iTIDNext - idHashTID[k]) / (iCDFNext - idHashCDF[k]);
    ...
    public long genOutDegreeSimple(long id) {
        ...
        } else if (id = odSimpleCurrNo + 1) {
            odSimpleCurrNo = id;
            if (odSimpleCurrId < odSimpleCNum.length) {
                if (odSimpleOffset < odSimpleCNum[odSimpleCurrId]) {
                    odSimpleOffset += 1;
                } else {
                    odSimpleOffset = 1;
                    odSimpleCurrId += 1;
    ...
    public long genOutDegreeComplex(long id) {
        double rv = ((id + 1) / bucket) * bucketStep + rand.nextDouble() * bucketStep;
        long index = Math.min(oHF(rv), odComplexDegree.length - 1);
    ...
    public long genTargetID() {
        double rv = Math.min(rand.nextDouble(), iMinRV);
        long index = iHF(rv);
        long a;
        double c, r;
